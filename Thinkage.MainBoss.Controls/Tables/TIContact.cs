...
namespace Thinkage.MainBoss.Controls {
	...
	public class TIContact : TIGeneralMB3 {
		...
		/// <param name="pathToRequestor"></param>
		/// <returns></returns>
		internal static TblContainerNode SingleRequestorLanguagePreferenceGroup(DBI_Path pathToRequestor) {
		...
		private class MergeContactCommand : ICommand {
			...
			public void Execute() {
				...
				using (var ds = new dsMB(Browser.DB)) {
					...
					toCode = ContactAsString(ds.T.Contact.Rows[0].F.Hidden, ds.T.Contact.Rows[0].F.Code);
				}
				...
				if (Ask.Question(BuildQuestion(toCode, ContactsToBeDeleted)) != Ask.Result.Yes)
					return;
				using (DBDataSet dsmerge = DBDataSet.New(Browser.TInfo.Schema.Database, Browser.DB)) {
					dsmerge.EnforceConstraints = false;
					dsmerge.DataSetName = KB.I("BrowseBaseControl.MergeContact.dsMove");
					var dbuser = dsmerge.DB.ConnectionInfo.ConnectionInformation.UserIdentification;
					...
					batch.Commands.Add(new DBSpecificCommandSpecification(mergeSql));
			...
			private static string BuildQuestion(string toContact, List<string> fromContacts) {
				...
				question.AppendLine();
				question.AppendLine(Strings.Format(KB.K("This operation can take a very long time.")));
				question.AppendLine();
...
")));
				question.AppendLine();
				question.AppendLine(Strings.Format(KB.K("If you or other users are currently using the contact which is the source of the merge, the merge may cause transient errors in MainBoss. Stopping and restarting MainBoss will fix the problem")));
				question.AppendLine();
	...
	-- Update tables that do not have derived records.
		...
		update Vendor set payablescontactid = @keepContact where payablescontactid = @deletecontacta
		...
		if( (select count(*) from #mappings ) != 0 ) begin
			...
			update MeterReading                 set Userid = keep from MeterReading                join #mappings on remove = Userid
...
";
			...
			public Libraries.Translation.Key Tip {
				get {
					if (Enabled)
						return KB.K("Merge all contact information from the selected contacts the target contact");
					else
						return Disabler.Tip;
				}
			}
			...
			readonly SettableDisablerProperties MergeTargetDisabler = new SettableDisablerProperties(null, KB.K("Select a contact to be merged into"), false);
			readonly SettableDisablerProperties SingleRecordDisabler = new SettableDisablerProperties(null, KB.K("Select a contact to be merged from"), false);
			readonly SettableDisablerProperties MultipleRecordDisabler = new SettableDisablerProperties(null, KB.K("Only one Contact at a time can be merged"), true);
			readonly PermissionDisabler PermissionToMergeDisabler = ((PermissionDisabler)Application.Instance.GetInterface<ITblDrivenApplication>().PermissionsManager.GetPermission(Root.Rights.Action.MergeContacts));
		...
		static TIContact() {
			...
			ContactFunctionsBrowsetteTblCreator = new DelayedCreateTbl(delegate () {
				...
				return new CompositeTbl(dsMB.Schema.T.ContactFunctions, TId.ContactFunction,
					...
					new CompositeView(dsMB.Path.T.ContactFunctions.F.RequestorID,               // Table 1 (Requestor)
						...
						CompositeView.PathAlias(dsMB.Path.T.ContactFunctions.F.ContactID, dsMB.Path.T.Requestor.F.ContactID)),
					new CompositeView(dsMB.Path.T.ContactFunctions.F.BillableRequestorID,       // Table 2 (BillableRequestor)
						CompositeView.RecognizeByValidEditLinkage(),
						RecordTypeViewColumnValue(),
						CompositeView.PathAlias(dsMB.Path.T.ContactFunctions.F.ContactID, dsMB.Path.T.BillableRequestor.F.ContactID)),
					new CompositeView(dsMB.Path.T.ContactFunctions.F.EmployeeID,                // Table 3 (Employee)
						CompositeView.RecognizeByValidEditLinkage(),
						RecordTypeViewColumnValue(),
						CompositeView.PathAlias(dsMB.Path.T.ContactFunctions.F.ContactID, dsMB.Path.T.Employee.F.ContactID)),
					new CompositeView(dsMB.Path.T.ContactFunctions.F.VendorID,
						CompositeView.RecognizeByValidEditLinkage(),
						CompositeView.IdentificationOverride(TId.VendorSalesContact),
						CompositeView.AddRecognitionCondition(new SqlExpression(dsMB.Path.T.ContactFunctions.F.VendorID.F.SalesContactID).EqNSV(new SqlExpression(dsMB.Path.T.ContactFunctions.F.ContactID))
														.And(new SqlExpression(dsMB.Path.T.ContactFunctions.F.VendorID.F.PayablesContactID).NEqNSV(new SqlExpression(dsMB.Path.T.ContactFunctions.F.ContactID))
														.And(new SqlExpression(dsMB.Path.T.ContactFunctions.F.VendorID.F.ServiceContactID).NEqNSV(new SqlExpression(dsMB.Path.T.ContactFunctions.F.ContactID))))
														),
						RecordTypeViewColumnValue(),
						NoNewMode),
					new CompositeView(dsMB.Path.T.ContactFunctions.F.VendorID,
						CompositeView.RecognizeByValidEditLinkage(),
						CompositeView.IdentificationOverride(TId.VendorServiceContact),
						CompositeView.AddRecognitionCondition(new SqlExpression(dsMB.Path.T.ContactFunctions.F.VendorID.F.SalesContactID).NEqNSV(new SqlExpression(dsMB.Path.T.ContactFunctions.F.ContactID))
														.And(new SqlExpression(dsMB.Path.T.ContactFunctions.F.VendorID.F.PayablesContactID).NEqNSV(new SqlExpression(dsMB.Path.T.ContactFunctions.F.ContactID))
														.And(new SqlExpression(dsMB.Path.T.ContactFunctions.F.VendorID.F.ServiceContactID).EqNSV(new SqlExpression(dsMB.Path.T.ContactFunctions.F.ContactID))))
														),
						RecordTypeViewColumnValue(),
						NoNewMode),                  // Table 4 (Sales Vendor)
					new CompositeView(dsMB.Path.T.ContactFunctions.F.VendorID,
						CompositeView.RecognizeByValidEditLinkage(),
						CompositeView.IdentificationOverride(TId.VendorAccountingContact),
						CompositeView.AddRecognitionCondition(new SqlExpression(dsMB.Path.T.ContactFunctions.F.VendorID.F.SalesContactID).NEqNSV(new SqlExpression(dsMB.Path.T.ContactFunctions.F.ContactID))
														.And(new SqlExpression(dsMB.Path.T.ContactFunctions.F.VendorID.F.PayablesContactID).EqNSV(new SqlExpression(dsMB.Path.T.ContactFunctions.F.ContactID))
														.And(new SqlExpression(dsMB.Path.T.ContactFunctions.F.VendorID.F.ServiceContactID).NEqNSV(new SqlExpression(dsMB.Path.T.ContactFunctions.F.ContactID))))
														),
						RecordTypeViewColumnValue(),
						NoNewMode),// Table 5 (Service Vendor)
					new CompositeView(dsMB.Path.T.ContactFunctions.F.VendorID,
						CompositeView.RecognizeByValidEditLinkage(),
						CompositeView.IdentificationOverride(TId.VendorSalesServiceContact),
						CompositeView.AddRecognitionCondition(new SqlExpression(dsMB.Path.T.ContactFunctions.F.VendorID.F.SalesContactID).EqNSV(new SqlExpression(dsMB.Path.T.ContactFunctions.F.ContactID))
														.And(new SqlExpression(dsMB.Path.T.ContactFunctions.F.VendorID.F.PayablesContactID).NEqNSV(new SqlExpression(dsMB.Path.T.ContactFunctions.F.ContactID))
														.And(new SqlExpression(dsMB.Path.T.ContactFunctions.F.VendorID.F.ServiceContactID).EqNSV(new SqlExpression(dsMB.Path.T.ContactFunctions.F.ContactID))))
														),
						RecordTypeViewColumnValue(),
						NoNewMode),// Table 6 (Accounts Payable Vendor)
					new CompositeView(dsMB.Path.T.ContactFunctions.F.VendorID,
						CompositeView.RecognizeByValidEditLinkage(),
						CompositeView.IdentificationOverride(TId.VendorSalesAccountingContact),
						CompositeView.AddRecognitionCondition(new SqlExpression(dsMB.Path.T.ContactFunctions.F.VendorID.F.SalesContactID).EqNSV(new SqlExpression(dsMB.Path.T.ContactFunctions.F.ContactID))
														.And(new SqlExpression(dsMB.Path.T.ContactFunctions.F.VendorID.F.PayablesContactID).EqNSV(new SqlExpression(dsMB.Path.T.ContactFunctions.F.ContactID))
														.And(new SqlExpression(dsMB.Path.T.ContactFunctions.F.VendorID.F.ServiceContactID).NEqNSV(new SqlExpression(dsMB.Path.T.ContactFunctions.F.ContactID))))
														),
						RecordTypeViewColumnValue(),
						NoNewMode),// Table 7 (Sales & Service Vendor)
					new CompositeView(dsMB.Path.T.ContactFunctions.F.VendorID,
						CompositeView.RecognizeByValidEditLinkage(),
						CompositeView.IdentificationOverride(TId.VendorServiceAccountingContact),
						CompositeView.AddRecognitionCondition(new SqlExpression(dsMB.Path.T.ContactFunctions.F.VendorID.F.SalesContactID).NEqNSV(new SqlExpression(dsMB.Path.T.ContactFunctions.F.ContactID))
														.And(new SqlExpression(dsMB.Path.T.ContactFunctions.F.VendorID.F.PayablesContactID).EqNSV(new SqlExpression(dsMB.Path.T.ContactFunctions.F.ContactID))
														.And(new SqlExpression(dsMB.Path.T.ContactFunctions.F.VendorID.F.ServiceContactID).EqNSV(new SqlExpression(dsMB.Path.T.ContactFunctions.F.ContactID))))
														),
						RecordTypeViewColumnValue(),
						NoNewMode),// Table 8 (Sales & Accounts Payable Vendor)
					new CompositeView(dsMB.Path.T.ContactFunctions.F.VendorID,// Table 10 (Sales, Service & Accounts Payable Vendor)
						CompositeView.RecognizeByValidEditLinkage(),
						CompositeView.AddRecognitionCondition(new SqlExpression(dsMB.Path.T.ContactFunctions.F.VendorID.F.SalesContactID).EqNSV(new SqlExpression(dsMB.Path.T.ContactFunctions.F.ContactID))
														.And(new SqlExpression(dsMB.Path.T.ContactFunctions.F.VendorID.F.PayablesContactID).EqNSV(new SqlExpression(dsMB.Path.T.ContactFunctions.F.ContactID))
														.And(new SqlExpression(dsMB.Path.T.ContactFunctions.F.VendorID.F.ServiceContactID).EqNSV(new SqlExpression(dsMB.Path.T.ContactFunctions.F.ContactID))))
						...
						RecordTypeViewColumnValue(),
						CompositeView.ContextFreeInit(dsMB.Path.T.ContactFunctions.F.ContactID, dsMB.Path.T.Vendor.F.SalesContactID),
						CompositeView.ContextFreeInit(dsMB.Path.T.ContactFunctions.F.ContactID, dsMB.Path.T.Vendor.F.ServiceContactID),
						CompositeView.ContextFreeInit(dsMB.Path.T.ContactFunctions.F.ContactID, dsMB.Path.T.Vendor.F.PayablesContactID)),// Table 9 (Service & Accounts Payable Vendor)
					new CompositeView(dsMB.Path.T.ContactFunctions.F.RequestAssigneeID,     // Table 11 (RequestAssignee)
						CompositeView.RecognizeByValidEditLinkage(),
						RecordTypeViewColumnValue(),
						CompositeView.PathAlias(dsMB.Path.T.ContactFunctions.F.ContactID, dsMB.Path.T.RequestAssignee.F.ContactID),
					...
					new CompositeView(dsMB.Path.T.ContactFunctions.F.WorkOrderAssigneeID,   // Table 12(WorkOrderAssignee)
						CompositeView.RecognizeByValidEditLinkage(),
						RecordTypeViewColumnValue(),
						CompositeView.PathAlias(dsMB.Path.T.ContactFunctions.F.ContactID, dsMB.Path.T.WorkOrderAssignee.F.ContactID),
						CompositeView.NewCommandGroup(assigneeGroup)),
					new CompositeView(dsMB.Path.T.ContactFunctions.F.PurchaseOrderAssigneeID,// Table 13 (PurchaseOrderRequestAssignee)
						CompositeView.RecognizeByValidEditLinkage(),
						RecordTypeViewColumnValue(),
						CompositeView.PathAlias(dsMB.Path.T.ContactFunctions.F.ContactID, dsMB.Path.T.PurchaseOrderAssignee.F.ContactID),
						CompositeView.NewCommandGroup(assigneeGroup)),
					new CompositeView(dsMB.Path.T.ContactFunctions.F.UserID,                // Table 14 (User)
						CompositeView.RecognizeByValidEditLinkage(),
						RecordTypeViewColumnValue(),
						CompositeView.PathAlias(dsMB.Path.T.ContactFunctions.F.ContactID, dsMB.Path.T.User.F.ContactID),
						NoNewMode)
				);
			});
			#endregion
			#region Contact from Active Directoryu
			...
			#endregion
			#region Contact
			DefineTbl(dsMB.Schema.T.Contact, () => {
				...
				new TblLayoutNodeArray(
					...
					BrowsetteTabNode.New(TId.ContactFunction, TId.Contact,
						TblColumnNode.NewBrowsette(TIContact.ContactFunctionsBrowsetteTblCreator, dsMB.Path.T.ContactFunctions.F.ContactID, DCol.Normal, ECol.Normal)),
			...
			ContactWithMergeTbl = new DelayedCreateTbl(() => {
				...
				return
				new Tbl(dsMB.Schema.T.Contact, TId.Contact,
				new Tbl.IAttr[] {
						ContactsDependentGroup,
						// Note that there was up until Oct 24/08 a ContactBrowseControl which contained #if'ed-off inoperational code to import from Outlook Contacts
						new BTbl(
								...
								delegate (BrowseLogic browserLogic) {
									...
									browserLogic.Commands.AddCommand(KB.K("Merge into Contact"), null, cmd, null, null,
											TblUnboundControlNode.New(KB.K("Merge to"), dsMB.Schema.T.Contact.F.Id.EffectiveType,
												...
												Fmt.SetPickFrom(dsMB.Schema.T.Contact, new SettableDisablerProperties(KB.K("Choose the contact which the selected contacts will be merged into"), KB.K("Select a contact to be merged to"),true)),
												Fmt.SetCreated(
