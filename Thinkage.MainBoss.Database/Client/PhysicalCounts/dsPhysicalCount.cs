	...
	public class dsPhysicalCount : DBDataSet {
		...
		protected dsPhysicalCount(SerializationInfo info, StreamingContext context) {
			...
			if ((strSchema != null)) {
				...
				if ((ds.Tables.FindTableDuringTableConstruction(SchemaClass.Instance.T.PhysicalCount) != null)) {
					this.Tables.Add(new PhysicalCountDataTable(ds.Tables.FindTableDuringTableConstruction(SchemaClass.Instance.T.PhysicalCount)));
				}
		...
		public class TAccessor {
			public TAccessor( dsPhysicalCount this_dataset ) {
				this.PhysicalCount = (PhysicalCountDataTable)this_dataset.Tables.FindTableDuringTableConstruction(SchemaClass.Instance.T.PhysicalCount);
			}
		...
		[Thinkage.Libraries.Translation.Invariant]
		public class PhysicalCountDataTable : DBICustomDataTable<PhysicalCountRow> {
			#region Construction
...
#if SerializationSupport
			internal PhysicalCountDataTable(System.Data.DataTable table) :
			...
			internal class FAccessor {
				public FAccessor(PhysicalCountDataTable t) {
				  this.ID = t.Columns.FindColumnDuringTableConstruction(SchemaClass.Instance.T.PhysicalCount.F.ID);
				  this.StoreroomAssignmentID = t.Columns.FindColumnDuringTableConstruction(SchemaClass.Instance.T.PhysicalCount.F.StoreroomAssignmentID);
				  this.Quantity = t.Columns.FindColumnDuringTableConstruction(SchemaClass.Instance.T.PhysicalCount.F.Quantity);
				  this.Cost = t.Columns.FindColumnDuringTableConstruction(SchemaClass.Instance.T.PhysicalCount.F.Cost);
				}
				/// <summary>
			...
			#endif
			#endregion
			protected override DBIDataTable CreateDBIInstance() => new PhysicalCountDataTable(Schema, Server);
			public PhysicalCountRow AddNewRow() => (PhysicalCountRow)DataSet.DB.AddNewRowAndBasesForGeneratedClass(DataSet, dsPhysicalCount.Schema.T.PhysicalCount);
			protected override DBIDataRow NewDBIRowFromBuilder(System.Data.DataRowBuilder builder) => new PhysicalCountRow(builder);
			public new dsPhysicalCount DataSet => (dsPhysicalCount)base.DataSet;
		}
		#region PhysicalCountRow
		[Thinkage.Libraries.Translation.Invariant]
		public class PhysicalCountRow : DBICustomDataRow<PhysicalCountDataTable> {
			#region Construction
			internal PhysicalCountRow(System.Data.DataRowBuilder rb)
				: base(rb) {
			}
			#endregion
			#region Typed Column accessor
