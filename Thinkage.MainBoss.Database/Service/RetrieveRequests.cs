		...
		public void Run(bool traceDetails) {
			...
			try {
				using (var messageSource = new EmailMessageSource(Logger, traceDetails, serverType, encryption, server, port??0, user, pw, mailbox)) {
					...
					int deleteCount = 0;
					foreach (var message in messageSource.Messages) {
						...
						try {
							...
							ServiceUtilities.EmailRequestFromEmail(dsmb, message, maxMailSize);
							delete = true;
							DB.Update(dsmb);
					...
					try {
						...
						GeneralException ge;
						switch (deleteCount) {
							default:		ge = new GeneralException(e, KB.K("Error deleting {0} email messages"), deleteCount);	break;
							case 1:			ge = new GeneralException(e, KB.K("Error deleting email message"));						break;
							case 0:			ge = new GeneralException(e, KB.K("Error closing email message source"));				break;
						}
						messageError(messageSource, ge, true);
					}
				}
