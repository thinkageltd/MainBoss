			...
			try {
				using (var messageSource = new EmailMessageSource(Logger, traceDetails, connectInfo, mailbox, encryption == DatabaseEnums.MailServerEncryption.RequireEncryption)) {
					...
					if (!Force && messageSource.Messages.Where(m => m.Sent < staleDate).Count() * 3 > n * 2) {
						...
						Logger.LogError(KB.K("No e-mails were processed because more that two thirds of them were stale; This may be the wrong mailbox.").Translate());
						return;
					}
					...
					foreach (var message in copyOfMessages) {
						...
						try {
							...
							if (!string.IsNullOrWhiteSpace(messageID) && MessageIDs.TryGetValue(messageID, out MessageUniqueness duplicateId) && message.SentAsString == duplicateId.SentAsText) {
