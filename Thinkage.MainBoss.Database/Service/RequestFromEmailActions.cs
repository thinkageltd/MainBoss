...
namespace Thinkage.MainBoss.Database.Service {
	public static class RequestFromEmailActions {
		...
		public static void Reject(MB3Client DB, Set<object> EmailRequestIds) { // there is a critical section here, but not important
			if (EmailRequestIds.Count() == 0) return; 
			...
			using (dsMB ds = new dsMB(DB)) {
				...
				foreach (dsMB.EmailRequestRow r in ds.T.EmailRequest.Rows) {
					...
					if (!emailAddresses.Contains(address)) {
						var requestorInfo = new AcquireRequestor((MB3Client)DB, new System.Net.Mail.MailAddress(r.F.RequestorEmailAddress, r.F.RequestorEmailDisplayName), forceCreate, forceCreate, forceCreate, null, null, r.F.PreferredLanguage);
				...
				foreach (dsMB.EmailRequestRow r in ds.T.EmailRequest.Rows) {
					try {
						Guid requestorId;
						if (!requestorIDs.TryGetValue(r.F.RequestorEmailAddress.ToLower(), out requestorId) )
							continue;
