...
namespace Thinkage.MainBoss.Database.Security {
	...
	public class RoleRight : NameAndNameSpaceAttribute {
		...
		private readonly List<string> pCostRights = new List<string>();
		/// <summary>
		...
		private readonly List<string> pTransitionRights = new List<string>();
		/// <summary>
		...
		private readonly List<string> pActionRights = new List<string>();
		...
		private readonly List<Include> pIncludes = new List<Include>();
	...
	public class RightSet {
		...
		private readonly Libraries.XAF.Database.Layout.DBI_Database pDatabaseSchema;
		#endregion
		#region Constructor
		...
		public IEnumerable<RoleAndPermission> RolesAndPermissions {
			get {
				return from p in Rights
					   let r = (RoleRight)p
					   where p.Class == TableRightType.Right || p.Class == TableRightType.Role
		...
		public IEnumerable<RoleAndPermission> RolesAndPermissionsFor(SecurityRoleIDs[] RoleIDs) {
			return from p in Rights
				   let r = (RoleRight)p
				   where p.Class == TableRightType.Role && System.Array.Exists(RoleIDs, (a => (int)a == r.Id))
		...
		private readonly Thinkage.Libraries.Collections.Set<string> TablesWithoutAnyPermissions = new Libraries.Collections.Set<string>();
#endif
	}
	#endregion
}
