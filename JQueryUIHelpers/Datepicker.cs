        ...
        public override string ToHtmlString()
        {
            ...
            if (!m_Inline)
            {
                return m_HtmlHelper.TextBox(m_Name,
                    m_Value == null ? null : m_Value.Value.ToString(FormattingString),
                    m_HtmlAttributes).ToHtmlString();
            }
            else
            {
                m_HtmlAttributes[DatepickerAttributeName.HiddenValue] = HtmlHelper.GenerateIdFromName(fullName);
				string hidden = m_HtmlHelper.Hidden(m_Name, m_Value == null ? null : m_Value.Value.ToString(FormattingString)).ToHtmlString();
