<#
.SYNOPSIS
Manage the objects necessary in Microsoft Entra Azure AD and Microsoft Exchange that allow the MainBoss Service to fetch mail using POP or IMAP
with OAuth2 authentication.
There must be an existing MainBoss Service Configuration in the MainBoss database.

.DESCRIPTION
This command script manages the objects required to provide the MainBoss Service with access to a Microsoft Exchange mailbox to
...
if (![String]::IsNullOrEmpty($SqlConnectString)) {	
	...
	{ 
		...
		if( $MBUtility -eq $null ) {
			throw ("Cannot find MBUtility.exe under "+$MainBossInstallDirectory)
		}
		"Using: "+$MBUtility
	}
...
The other prompt is to authorize changes to your Microsoft Exchange service to complete the changes required.`
"
