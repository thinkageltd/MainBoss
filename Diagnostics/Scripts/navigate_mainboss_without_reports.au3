...
;;; Change Me to avoid Certain Forms.
;;; This guy is the button that activates the form.
Global $_AvoidButton	= "Print"
;;; This guy is the static title that maybe found in the browsette.
Global $_AvoidStatic	= "Report"
...
Func _Main()
	...
	While _SQLite_FetchData ($Query, $aRow) = $SQLITE_OK
		...
		if _NavControl( $aRow[0], $aRow[1], $hDB ) == 2 Then ContinueLoop
...
;~ 			EndIf
			...
			_LogEvent("Sleeping for 200 mseconds")
			Sleep(200)
		EndIf
	WEnd
...
#cs----------------------------------------------------------------------------
	...
	Revisions		: October 17, 2007
						- Changed the method $Locations returns buttons.
							Overall output from this function remains.
						- Added a test for the button's text.  If it 'reads'
							"Print" then add one to the count, and continue
							the loop.
					  October 10, 2007
...
;	Local $_outFile =  $_windowTitle & ".ini"
	...
	For $a = 1 To $text2[0]
		...
		If StringInStr($text2[$a], "BUTTON", 2) AND _IsVisible(ControlGetHandle($_windowTitle, "", "[CLASS:" & $text2[$a] & "; INSTANCE:" & $b & "]")) Then
			$b += 1
			
			if StringInStr(ControlGetText($_windowTitle, "", "[CLASS:" & $text2[$a] & "; INSTANCE:" & $b & "]"), $_AvoidButton, 2) > 0 Then
				ContinueLoop
			EndIf
	...
	;~ 			iniwrite($_outFile, "Form", "button"&$b+1, ControlGetText($_windowTitle, "", "[CLASS:" & $text2[$a] & "; INSTANCE:" & $b+1 & "]"))
				...
				Local $szArray = UBound($Locations, 1)
				ReDim $Locations[$szArray + 1][2]
				
				$Locations[$szArray][0] = $text2[$a]
				$locations[$szArray][1] = $b
			Else
	;~ 			iniwrite($_outFile, "Form", "button"&$b+1, ControlGetText($_windowTitle, "", "[CLASS:" & $text2[$a] & "; INSTANCE:" & $b+1 & "]"))
				$Locations[0][1] = $_windowTitle
				$Locations[0][0] += 1
...
#cs----------------------------------------------------------------------------
	...
	Return Value	: 1 - Completed successfully.
					  2 - Loop was terminated early.
	Revisions		: October 17, 2007
						- Added trap for "Reports".  This was to stop the 
							report viewer from opening.
						- Added return values so the caller would know it
							was terminated early.
					  October 10, 2007
						- Added to the navigate_mainboss script for ease of
							compilation.
					  August 29, 2007
						- Completed coding.
#ce----------------------------------------------------------------------------
Func _NavControl(const $_Destination, const $_Action, const $hDB)
	...
	;; Test that The static title changed ::: if not, then coninute to wait until it does
			...
			;;; Added this to easily jump out of this function and to return to the caller--stopping the report viewer.
			if StringInStr($var[3], $_AvoidStatic, 2) > 0 Then Return 2
