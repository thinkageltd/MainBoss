...
namespace Thinkage.MainBoss.MBUtility {
	...
	internal class AddContactFromLDAP {
		public class Definition : UtilityVerbWithDatabaseDefinition {
			...
			public override string Verb {
				...
				get {
					return "AddContactfromactivedirectory";
				}
			}
			public override void RunVerb() {
				new AddContactFromLDAP(this).Run();
			}
		}
		private AddContactFromLDAP(Definition options) {
			Options = options;
		}
		...
		private void Run() {
			if (!LDAPEntry.IsInDomain())
				throw new GeneralException(KB.K("'{0}' can only work if the computer is in a domain; if you are in a domain, then the domain controller is currently inaccessible"), KB.I("AddContactfromactivedirectory"));
			string oName;
			...
			try {
				...
				dbapp.SetVersionHandler(MBUpgrader.UpgradeInformation.CheckDBVersion(session, VersionInfo.ProductVersion, minDBVersionForRolesTable, dsMB.Schema.V.MinMBAppVersion, KB.I("MainBoss Utility Tool--Create Contact")));
			...
			foreach (var EmailAddress in EmailAddresses) {
				...
				using (dsMB updateDs = new dsMB(db)) {
					var ContactInfo = new AcquireContact(db, null, se, true, true, null);
